...
export const saveFileTool = {
    name: "save_file",
    description: "保存内容到指定类型的文件夹。⚠️ 重要限制：content 单次最多 600 字符（约 300 汉字），超过会导致 JSON 解析失败。长内容必须分段：1) 先用 save_file 保存前 600 字；2) 再用 append_to_file 多次追加，每次不超过 600 字。",
    parameters: {
        type: "object",
        properties: {
            type: {
...
export const generateLongContentTool = {
    ...
    description: "🚀 革命性工具！用于生成长章节（3000+ 字）。参数是创作指令（prompt），不是完整内容。工具内部自动调用 LLM 生成纯文本，实时聚合，自动分段保存。一次调用完成整章创作，无 JSON 长度限制！",
    parameters: {
        type: "object",
        properties: {
            type: {
    ...
    func: async (input) => {
        try {
            ...
            const filename = `${timestamp}Z_${title}.md`;
